...
| F6 | Unverifiable/likely-hallucinated citations ("SAM 3.1 Object Multiplex", Impact Scores, unverified Point-SAM/VESPA numbers) | Credibility | Citation verification checklist (§7.1.1): every model, venue, and number is re-derived from the primary source or dropped. Unverifiable components are replaced or demoted to `[EXPERIMENTAL]` | §7.1 |
...
| "SAM 3.1", "Object Multiplex" | Not verifiable as of drafting | **Replaced** by **SAM 2.1** (open weights) for mask generation + video propagation with memory; multi-object handled by batched per-object state `[VERIFY version + memory-window settings]` |
